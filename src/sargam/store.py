...
class Store:
    ...
    # Below this the replay costs less than reading and writing the blob.
    SNAPSHOT_MIN_POINTS = 200

    def close(self) -> None:
        try:
            self._save_snapshot()
        except Exception:
            # A snapshot is an optimisation. Failing to write one must never
            # stop a store from closing cleanly, because the next open can
            # always replay instead.
            pass
        self.db.commit()
        self.db.close()
    ...
    def _fingerprint(self) -> str:
        """Identifies the live constraint set exactly. Anything that changes
        which rows are live changes this, so a stale matrix can never be
        mistaken for a current one."""
        h = hashlib.sha256()
        for r in self.db.execute(
                "SELECT id FROM constraints WHERE retracted = 0 ORDER BY id"):
    ...
    def _save_snapshot(self) -> bool:
        ...
        if tl is None or tl._n < self.SNAPSHOT_MIN_POINTS:
            return False
        D = np.ascontiguousarray(tl.D, dtype=np.float64)
        self.db.execute(
            "INSERT INTO solver_snapshot (id, n_points, fingerprint, matrix, "
            "updated_at) VALUES (1,?,?,?,?) "
            "ON CONFLICT(id) DO UPDATE SET n_points=excluded.n_points, "
            "fingerprint=excluded.fingerprint, matrix=excluded.matrix, "
            "updated_at=excluded.updated_at",
            (tl._n, self._fingerprint(), D.tobytes(), now()),
        )
        self.db.commit()
        return True

    def _snapshot_for(self, n_points: int) -> np.ndarray | None:
        row = self.db.execute(
            "SELECT n_points, fingerprint, matrix FROM solver_snapshot "
            "WHERE id = 1").fetchone()
        if row is None or row["n_points"] != n_points:
            return None
        if row["fingerprint"] != self._fingerprint():
            return None
        buf = row["matrix"]
        if len(buf) != n_points * n_points * 8:
            return None
    ...
    def _load_timeline(self) -> Timeline:
        ...
        snap = self._snapshot_for(tl._n)
        if snap is not None:
            # The fingerprint says this matrix belongs to exactly these rows,
            # so the replay below would reproduce it edge for edge.
            tl.adopt([Constraint(r["x_point"], r["y_point"],
                                 _dec(r["lo_days"]), _dec(r["hi_days"]),
                                 r["provenance"], r["source"] or "",
                                 r["note"] or "") for r in rows], snap)
            return tl
