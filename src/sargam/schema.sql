...
-- The solved closure, cached. Opening a store replays every constraint
-- through the incremental relaxation, which is O(m*n^2) and is the dominant
-- cost of a cold open once stores are evicted from memory. The fingerprint is
-- over the live constraint rows: if it still matches, the matrix is exactly
-- what a replay would have produced.
CREATE TABLE solver_snapshot (
