...
class Timeline:
    ...
    def add(self, x: int, y: int, lo: float, hi: float,
        ...
        if check and not self._admits(x, y, lo, hi):
    ...
    def _admits(self, x: int, y: int, lo: float, hi: float) -> bool:
        """Whether x - y in [lo, hi] can hold, decided before anything moves.

        The closure already proves x - y lies in [-D[x][y], D[y][x]]. A new
        assertion on the same pair is satisfiable exactly when its interval
        meets that one, and unsatisfiable otherwise -- so consistency is two
        comparisons against cells we already have, not a search for a negative
        cycle after the fact.

        This replaces a full copy of the distance matrix per constraint, taken
        so a rejected assertion could be rolled back. That copy is O(n^2) in
        memory traffic and, at a few thousand events, was the single largest
        cost in the system: building a 1500-event network moved a quarter of a
        terabyte through memory to guard against a case that is rare by
        construction. Nothing is mutated now unless it is already known to be
        admissible, so there is nothing to roll back.
        """
        D = self.D
        upper = D[y, x]          # proven maximum of x - y
        lower = -D[x, y]         # proven minimum of x - y
        # NaN from inf - inf means the pair is unconstrained in that direction;
        # an unconstrained bound cannot rule anything out.
        if not (np.isnan(upper) or lo <= upper + 1e-9):
            return False
        if not (np.isnan(lower) or hi >= lower - 1e-9):
            return False
        return True
    ...
    def adopt(self, constraints: list[Constraint], D: np.ndarray) -> None:
        """Install a closure that was solved earlier instead of replaying the
        constraints that produced it.

        Replaying is O(m*n^2) and is the dominant cost of opening a store that
        has been evicted from memory, which in a hosted setting is most opens.
        The caller is responsible for establishing that `D` belongs to exactly
        this constraint set -- the store does that with a fingerprint over the
        live constraint rows. What is checked here is only what is cheap to
        check: a closure with a negative diagonal is an inconsistent network,
        and that must never be adopted silently.
        """
        n = self._n
        if D.shape != (n, n):
            raise ValueError(f"closure is {D.shape}, network has {n} points")
        diag = np.diagonal(D)
        if (diag < -1e-9).any():
            raise ValueError("refusing to adopt an inconsistent closure")
        self._D[:n, :n] = D
